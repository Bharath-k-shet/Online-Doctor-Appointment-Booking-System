...
//       <MoveUpOnRender id="my-appointments">
//         {appointments
//           .filter((item) => item?.docData) // ✅ FIX: avoid null error
//           .map((item, index) => (
//             <div
//               className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b"
//               key={index}
...
//               <div className="flex flex-col gap-2 justify-end">
//                 {!item.cancelled && item.payment && !item.isCompleted && (
//                   <button className="text-sm text-stone-500 sm:min-w-48 py-2 border bg-indigo-50">
//                     Paid
//                   </button>
//                 )}
...
//                   <button
//                     onClick={() => appointmentRazorpay(item?._id)}
//                     className="text-sm text-stone-500 sm:min-w-48 py-2 border hover:bg-primary hover:text-white duration-300"
//                   >
//                     Pay Online
//                   </button>
//                 )}
...
//                   <button
//                     onClick={() => cancelAppointment(item._id)}
//                     className="text-sm text-stone-500 sm:min-w-48 py-2 border hover:bg-red-600 hover:text-white duration-300"
//                   >
//                     Cancel Appointment
//                   </button>
//                 )}
...
const MyAppointments = () => {
  ...
  return (
    <div>
      ...
      <MoveUpOnRender id="my-appointments">
        {appointments
          .filter((item) => item?.docData) // ✅ avoid crash
          .map((item, index) => (
            <div
              className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b"
              key={index}
            >
              ...
              <div className="flex flex-col gap-2 justify-end">
                ...
                {!item.cancelled && item.payment && !item.isCompleted && (
                  <button className="text-sm sm:min-w-48 py-2 border bg-indigo-50 text-green-600 font-semibold">
                    Paid ✅
                  </button>
                )}
                ...
                {!item.cancelled && !item.payment && !item.isCompleted && (
                  <button
                    onClick={() => appointmentRazorpay(item?._id)}
                    className="text-sm sm:min-w-48 py-2 border hover:bg-primary hover:text-white duration-300"
                  >
                    Pay Online
                  </button>
                )}
                ...
                {!item.cancelled && !item.isCompleted && (
                  <button
                    onClick={() => cancelAppointment(item._id)}
                    className="text-sm sm:min-w-48 py-2 border hover:bg-red-600 hover:text-white duration-300"
                  >
                    Cancel Appointment
                  </button>
                )}
